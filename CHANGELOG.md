# Changelog

Формат основан на [Keep a Changelog](https://keepachangelog.com/ru/1.1.0/),
проект придерживается [SemVer](https://semver.org/lang/ru/).

## [Unreleased]

См. [ROADMAP.md](ROADMAP.md).

## [2.3.1] — 2026-04-27

### Исправлено
- Тримминг ведущих/хвостовых пробельных символов (пробел, таб, CR, LF)
  во всех интерактивных полях ввода имени инстанса/хранилища. Было:
  случайный хвостовой пробел из копипаста или CR из Windows-терминала
  валил валидацию `^[A-Za-z0-9_-]+$` сообщением «Недопустимое имя».
  Теперь скрипт молча обрезает пробелы на границах строки. На имена
  это не влияет (валидатор всё равно запрещает пробелы внутри).

  Затронутые места: `instance create`, `instance delete` (подтверждение
  имени), миграция legacy → multi-instance, `repo create`, `repo rename`,
  `repo delete` (подтверждение), `repo restore` (новое имя и
  подтверждение замены).

  Helper: `_trim_ws "$str"` — удаляет `[[:space:]]` слева и справа,
  оставляя содержимое и внутренние символы как есть.

## [2.3.0] — 2026-04-27

### Новое: подключение существующего каталога как хранилища
Меню «Хранилища → 9) Инициализировать хранилище из существующего
каталога» (`do_repo_adopt`). Решает две задачи:

1. **«Усыновить» осиротевший каталог** в `INST_REPO_DIR`, который
   скрипт фильтрует из списка хранилищ (rollback `*.pre-restore.*`,
   невалидное имя, остаток после прерванного restore). Скрипт сам
   находит такие каталоги и предлагает выбрать.

2. **Подключить произвольный каталог** на диске (например,
   перенесённое через `rsync` хранилище). На выбор: скопировать
   (источник остаётся) или переместить.

Алгоритм:
- Сканирует `INST_REPO_DIR` на «осиротевшие» каталоги.
- Предлагает выбрать из списка или указать произвольный путь.
- Подсказывает имя хранилища (по умолчанию — basename, для
  `*.pre-restore.*` срезает суффикс).
- Если каталог не похож на хранилище 1С (нет `1cv8ddb.lst`,
  `cache/`, `data/`, `v8inforeg.lst`) — спрашивает подтверждение.
- Останавливает службу, переносит/копирует, делает
  `chown usr1cv8:grp1cv8` и `chmod 750`, запускает службу.

### CLI команда `repo adopt`
```
crserver [-i name] repo adopt <путь> [--as <имя>] [--move|--copy] [--force]
```
- `--as <имя>` — имя хранилища (иначе из basename, с обрезкой
  `.pre-restore.*` суффикса).
- `--move` — переместить источник (по умолчанию `--copy`).
- Если `<путь>` уже внутри `INST_REPO_DIR` — всегда move (легализация).
- `--force` — подключить даже если нет признаков 1С-структуры
  (по умолчанию команда отказывает).
- bash-completion дополнен.

### Документация
- README получил примеры `repo adopt` со всеми сценариями.

## [2.2.3] — 2026-04-27

### Hotfix (КРИТИЧЕСКИЙ, потеря данных)
В сценарии «восстановить под другим именем» (v2.1.4–2.2.2) был
серьёзный баг: `tar -xzf` распаковывал архив **поверх живой
директории** хранилища с тем же исходным именем, после чего
`mv orig new` либо falling, либо приводил к смешанному содержимому.

Симптомы у пользователя:
- В меню «Хранилища» исходное хранилище исчезало
- Появлялась запись с новым именем, но без директории
- Директория старого имени оставалась с непредсказуемым содержимым

**Полностью переписан алгоритм через staging-каталог:**
1. `mktemp -d` создаёт `${INST_REPO_DIR}/.restore-staging.<rnd>` (на той
   же ФС, чтобы `mv` был атомарным).
2. `tar -xzf $archive -C $staging` распаковывает в staging — **живые
   данные не трогаются**.
3. Если `target` уже есть и мы заменяем — отодвигаем в `.pre-restore.<ts>`.
4. `mv staging/orig_name → target` — атомарно.
5. Удаляем staging.

При любой ошибке на любом шаге — staging-каталог удаляется,
.pre-restore возвращается обратно, скрипт стартует службу. Никакого
смешения данных больше не происходит.

Применено к **обоим** местам: `do_repo_restore` (меню) и CLI
`repo restore`.

### Качество отображения
- `get_repo_list` теперь фильтрует:
  - `*.pre-restore.*` rollback-каталоги — они не должны показываться
    как полноценные хранилища в меню;
  - `.*` (скрытые служебные);
  - имена, не проходящие `validate_repo_name`.

### Тесты
- T1 (restore as new): `myrepo` сохраняет live-data, `myrepo_restored`
  получает backup-data.
- T2 (replace): `myrepo` получает backup-data, live уходит в
  `myrepo.pre-restore.<ts>`.
- T3 (фильтрация): pre-restore не виден в списке.

## [2.2.2] — 2026-04-27

### Hotfix
- v2.2.1 заменил `grep "\|"` на `awk` для проверки top-level в архиве,
  но на серверах с **mawk** (Debian/Ubuntu по умолчанию) проверка
  снова отказывала на нормальных бэкапах. mawk не всегда корректно
  обрабатывает `index() == 1 && length(...)` в одном предикате.
- Третья итерация фикса: переписано на **чистый bash** (`case` с
  глобами, `while read` от `tar -tzf`). Не зависит ни от grep
  (BRE/ERE), ни от awk-реализации (gawk/mawk/busybox).
- Тесты на 7 кейсах: норм директория, файл вместо директории,
  только файлы внутри без trailing-slash entry, имя-префикс другого
  (foo vs foobar), реалистичный 1С-репозиторий с `1cv8ddb.lst`.

### Hotfix
- Регрессия v2.1.4: проверка «top-level в архиве — директория» падала
  ложноположительно на нормальных бэкапах. Причина: `grep` без `-E`
  трактовал альтернацию `\|` в BRE по-разному на разных системах
  (на Debian-сервере `\|` не работало как «или», а как литерал).
  Меню «Хранилища → Восстановить» отказывало с ошибкой
  «top-level не является директорией» даже для архивов, созданных
  тем же скриптом.
- Заменено на `awk`-проверку, которая не зависит от BRE/ERE и
  одинаково работает на всех системах. Покрытие тестов:
  - архив с `name/` trailing-slash entry — PASS;
  - архив только с файлами `name/...` без отдельной записи каталога — PASS;
  - архив, где `name` — это файл (без слеша) — правильно отклоняется;
  - имя-префикс другого имени (`foo` vs `foobar`) — без ложных матчей.

### Журнал ошибок
- Все вызовы `log_error` теперь дублируются в файл
  `/var/log/1c-crserver/manager.log` с timestamp, уровнем, именем
  функции и номером строки в скрипте, плюс контекст инстанса
  (`[inst=name]`), если он задан.
- Ротация по размеру: при превышении 5 MiB текущий файл сохраняется
  как `.1`, предыдущий `.1` уходит в `.2`, более старые удаляются.
- Сбой записи (нет прав, диск полон) **не падает в скрипте** — лог
  просто молча пропускается, чтобы не создавать вторичные ошибки.
- Файл создаётся только при первой реальной ошибке — если ошибок не
  было, нет и файла.

### Меню «Инструменты → Журнал ошибок» (пункт 7)
- Показать последние 50/N записей с цветной подсветкой уровня.
- Поиск по подстроке/regex (`grep -E`) с прохождением по архивам
  `.1` и `.2`.
- `tail -F` для слежения в реальном времени (переоткрывает файл
  при ротации).
- Очистка журнала со всеми архивами (с подтверждением).

### CLI команда `errors`
- `crserver errors` — последние 50 строк
- `crserver errors 200` — последние N
- `crserver errors --grep <pattern>` — поиск по архивам
- `crserver errors --follow` (`-f`) — tail -F
- `crserver errors --clear` — очистить
- `crserver errors --path` — напечатать путь к файлу
- `crserver errors --help` — справка
- bash-completion расширен для всех опций.

### Документация
- README получил раздел «Журнал ошибок» в Troubleshooting с примерами
  CLI, локацией файла и описанием ротации.

### Восстановление хранилищ
- В меню «Хранилища → Восстановить» добавлен **выбор сценария**, когда
  целевое имя уже существует:
  1. Заменить существующее (со страховочным `<имя>.pre-restore.<ts>`).
  2. Восстановить **под другим именем** (как новое хранилище). По умолчанию
     предлагается `<orig>_restored`, при коллизии — `_2`, `_3`, ...
  3. Отмена.
  Если целевого имени на диске нет — менеджер просто спросит, оставить
  исходное имя или ввести своё.
- Перед распаковкой проверяется, что top-level в архиве — **директория**
  (а не файл с тем же именем). Это закрывает класс багов, когда после
  «успешной» распаковки в `REPO_DIR` оказывался файл, а не каталог
  хранилища, и в списке «хранилище» появлялось без реальной директории.
- Добавлена post-condition проверка: после `tar -xzf` целевой путь
  должен быть директорией. Иначе делаем rollback и сообщаем явную ошибку.
- Защита от случайного `rm -rf /`: все `rm -rf "${INST_REPO_DIR}/${orig_name}"`
  заменены на `${var:?}`-форму, чтобы при пустых переменных `rm`
  падал с понятной ошибкой, а не разворачивался в опасный путь.

### CLI
- `repo restore` принимает новые опции:
  - `--as <new>` — восстановить под другим именем.
  - `--force` — заменить существующее без подтверждения (для
    автоматизации).
  Без `--force` восстановление в существующее имя теперь возвращает
  ошибку с подсказкой («используйте `--force` или `--as <new>`»).

### Документация
- README обновлён с примерами `repo restore --as` и `--force`.

### Hotfix
- Меню «Файрвол → Добавить разрешённый IP» падало с
  `BASH_REMATCH[$i]: unbound variable` под `set -u` сразу после ввода
  адреса. Причина — обращение к `${BASH_REMATCH[$i]}` без подстановки
  по умолчанию. На некоторых сборках bash 5.x под `set -u` это
  трактуется как unbound, даже когда regex успешно совпал.
  Исправлено в `validate_ip` (берём октеты сразу после `[[ =~ ]]` через
  `${BASH_REMATCH[N]:-}`). Та же страховка добавлена в `instance_load`
  и `get_legacy_install`, где парсится конфиг — на случай аналогичных
  edge-cases при чтении файла.

### Тесты
- `validate_ip` покрыт 17 локальными кейсами: обычные IPv4, loopback,
  all-zero/FF, подсети /0..32, граничные ошибки (octet > 255, /33,
  /-1, ведущие нули, неполный/лишний октет, пустая строка, не-IP).

### CI
- В git index `crserver-manager.sh` помечен как исполняемый (mode 100755).
  При checkout под Windows исполняемый бит не сохранялся, поэтому
  `./crserver-manager.sh` падал с `Permission denied` в smoke-test
  Docker-контейнера на Linux. Заодно smoke-test переписан на вызов через
  `bash crserver-manager.sh` для устойчивости к похожим проблемам.
- В smoke-test убран `apt-get update && install bash` — bash всегда
  есть в `debian:12`, а update мог зависнуть/упасть при сетевых
  проблемах CI и блокировать всю проверку.

## [2.1.1] — 2026-04-25

### Code quality
- Скрипт чисто проходит shellcheck с `--severity=warning` (как настроено
  в CI). Исправлены два предупреждения:
  - `SC2168` (error): `local _size _entries` в case-ветке `verify` была
    вне функции — заменено на обычные переменные с `unset` после.
  - `SC2034`: переменная `BLUE` была определена, но нигде не использовалась —
    удалена.
- В `contrib/crserver.bash-completion` добавлен `# shellcheck disable=SC2207`
  с комментарием о безопасности — `COMPREPLY=( $(compgen ...) )` это
  стандартная идиома bash-completion, имена в системе (инстансы, версии,
  хранилища) не содержат пробелов.

### Бэкапы
- Ротация по числу копий: пункт меню «Оставить только последние N» и
  выбор политики в `do_setup_cron_backup` («по возрасту» / «по числу
  копий»). Cron-скрипт теперь поддерживает оба механизма через
  переменные `KEEP_DAYS` и `KEEP_LAST`.
- Команда `crserver verify <архив.tar.gz>` — `tar -tzf`-проверка
  целостности любого архива. Возвращает exit 0 при успехе, 1 при
  повреждении. Печатает размер и число записей.
- Пункт меню «Проверить целостность архива (verify)» с интерактивным
  выбором архива из списка инстанса.
- `make_tar_archive` helper использует `pv` для прогресс-бара, если
  утилита установлена и stdout — tty. На non-tty (cron, pipe) `pv`
  пропускается. Helper применим к новым местам вызова tar.

### CI
- `.github/workflows/ci.yml` — три job:
  1. ShellCheck с `--severity=warning` (исключения SC2155/SC2128/SC1091).
  2. `bash -n` syntax check.
  3. Smoke-test в Docker debian:12 (запуск `version`, `help`).

### UX
- `crserver version` и `crserver help` больше не требуют root: эти
  команды перехватываются до `check_root`. Раньше любой невинный
  `crserver --version` падал с «Запустите от root».

### Code quality
- `confirm`-helper применён в первом call-site (удаление бэкапа
  по номеру). Миграция остальных идёт инкрементально.

## [2.0.4] — 2026-04-25

### Корректность
- `do_full_install` корректно обрабатывает наполовину созданный инстанс
  (после прерванной первой установки): предлагает либо открыть меню
  инстанса для запуска/настройки, либо удалить и переустановить с нуля.
  Раньше повторный запуск падал на «инстанс уже существует».
- `validate_ip` строго проверяет октеты 0–255 и маску /0–32. Раньше
  проходили строки вида `999.0.0.0/99` и числа с ведущими нулями.
- CLI `instance create` валидирует все аргументы: формат версии
  (`8.3.NN.NNNN`), её установленность, диапазон порта 1–65535, коллизию
  с другими инстансами, абсолютность путей и отсутствие переносов строк.

### UX
- Поддержка [`NO_COLOR`](https://no-color.org/) и автоматическое
  отключение цветов, когда stdout не tty (pipe в файл, cron-stdout).
- Единый `confirm` helper для Y/N-вопросов с уважением к stdin-не-tty
  (для CI берёт значение по умолчанию). Helper доступен новым call-sites,
  миграция существующих идёт инкрементально (см. ROADMAP).

### Автоматизация
- `crserver instance list --json` — машинно-читаемый список инстансов
  для Ansible/мониторинга. Поля: `name`, `status`, `version`, `port`,
  `repo_dir`, `log_dir`, `is_default`. Без зависимости от `jq`.
- `crserver [-i name] healthcheck` — однострочный `OK ... ` / `FAIL ...`
  с exit 0/1. Подходит для systemd `ExecStartPost=`, мониторинга,
  cron-проверок.
- `contrib/crserver.bash-completion` — bash-completion для всех команд,
  имён инстансов (после `-i`), установленных версий 1С (после
  `--version`) и хранилищ выбранного инстанса.

## [2.0.3] — 2026-04-25

### Безопасность операций
- `do_repo_backup`, `do_backup`, CLI `repo backup` обрабатывают SIGINT/SIGTERM:
  Ctrl+C во время `tar -czf` больше не оставляет недописанный архив на диске.
- `move_dir_safe` проверяет свободное место на целевой ФС через `df` перед
  копированием на другую файловую систему. Раньше копирование могло начаться
  и оборваться на середине, оставив частичный каталог.
- `get_legacy_install` возвращает 1, если в обнаруженном legacy-юните не
  удалось распарсить версию платформы. Повреждённая v1.x-установка больше
  не воспринимается как «обнаруженная» и не предлагается к миграции.

### UX
- При создании инстанса проверяется не только коллизия портов с другими
  инстансами, но и реальная занятость порта в системе через `ss -tlnH`.
- Понижение версии в `do_instance_edit` (`8.3.30 → 8.3.25`) требует явного
  подтверждения с предупреждением: после обновления формата хранилища
  конфигуратором понижение делает данные нечитаемыми.
- Опция `-i/--instance` принимается в любой позиции аргументов CLI:
  `crserver start -i dev30` теперь равнозначно `crserver -i dev30 start`.

### CLI / автоматизация
- `update --check` всегда возвращает exit 0 при успешном выполнении проверки
  (включая «уже актуальная версия»), и exit 1 только при сетевой/парсинговой
  ошибке. Раньше код 1 в обоих случаях сбивал cron-обёртки. Внутренние коды
  100/101 сохранены для интерактивного использования из меню.

## [2.0.2] — 2026-04-25

### Файрвол
- `chain_rule_count` всегда печатает число (awk-обёртка вместо
  `grep -c | true`). Раньше при пустой цепочке возвращалась пустая строка,
  и `add_allowed_ip` фолбэчил на ветку `-A`, нарушая порядок правил.
- `setup_firewall_chain` больше не сбрасывает существующую цепочку: при
  пересоздании инстанса whitelist-настройки администратора сохраняются.
- Per-instance цепочка валидирует имя через `instance_name_valid` и
  проверяет лимит iptables (28 символов) — раньше длинное имя инстанса
  ронял `iptables -N`.

### Валидация
- `instance_save` запрещает совпадение `REPO_DIR` и `LOG_DIR`, а также
  вложенность одного в другой: защита от `chown -R` и `rm -rf` на одном
  каталоге.
- Введён `version_name_valid` (`^8\.3\.[0-9]+\.[0-9]+$`), применяемый везде,
  где имя версии становится частью пути. Защита от path traversal в
  `/opt/1cv8/x86_64/<ver>/` и `packages/<ver>/`.

### Установка пакетов
- `install_version` ставит четыре `.deb` одной командой `apt-get install`:
  apt разруливает порядок и подтягивает зависимости (`libwebkit2gtk` и др.).
  Раньше серия `dpkg -i` падала на первой ошибке зависимостей. Старый путь
  (`dpkg -i` + `apt-get install -f`) сохранён как fallback.

### Прочее
- `do_full_uninstall` обёрнут в `set +e` вокруг цикла удаления инстансов:
  uninstall дочистит остальные инстансы даже если `awk`/`systemctl` упадёт
  на одном из них.
- `do_repo_check` явно проверяет `[[ -d "$dir" ]]` перед `stat`/`find`.
- ss-проверки порта переведены с `grep ":port"` на `awk` по 4-й колонке
  с якорем `$`. Раньше для порта 1542 матчились и 15420, 15425.
- В cron-скрипте бэкапа: `KEEP_DAYS` закавычен, как и `+${KEEP_DAYS}` в `find -mtime`.

## [2.0.1] — 2026-04-25

### Критические фиксы
- `do_instance_delete`: `cleanup_instance_chain` теперь вызывается **до**
  `rm -f conf` (и в меню, и в CLI). Функция читает порт из конфига через
  `awk` — при удалении конфига **до** очистки файрвола, per-instance
  цепочка оставалась прицепленной к `INPUT`.
- `do_repo_restore`: проверяется, что в архиве **ровно один** top-level
  каталог с допустимым именем хранилища. Раньше при несовпадении имён
  rollback мог оставить «осиротевший» pre-restore-каталог.
- CLI `repo delete`: `was_active`-логика отделена от exit-кода `rm -rf`.
  Если `rm` падал, служба больше не возвращалась под `set -e`. Добавлена
  валидация имени.

### Новое в CLI
- `repo info <имя>` — машинно-читаемая сводка по хранилищу.
- `repo rename <старое> <новое>` — переименование с остановкой/запуском службы.
- `repo restore <архив | имя_хранилища>` — восстановление по полному пути
  к архиву или по имени хранилища (берётся свежайший бэкап в `BACKUP_DIR`).

### Целостность бэкапов
- После `tar -czf` в `do_repo_backup`, `do_backup`, CLI `repo backup` и
  cron-скрипте выполняется `tar -tzf` для проверки целостности.
  Битый архив удаляется немедленно с записью в лог.

## [2.0.0] — мульти-инстанс архитектура

Полная переработка архитектуры. Один сервер обслуживает несколько независимых
инстансов через systemd-template `crserver@<имя>.service`. Каждый инстанс —
свой конфиг в `/etc/1c-crserver/instances/<имя>.conf` (VERSION, REPO_PORT,
REPO_DIR, LOG_DIR), своя версия платформы, свои хранилища, свои бэкапы.
Файрвол: общая цепочка `CRSERVER` + опциональные `CRSERVER-<имя>` для
тонких per-instance ACL. Автомиграция со старой v1.x установки в инстанс
`default` через пункт меню «Инстансы → Миграция».

## [1.4.0] и ранее

См. историю в `git log`.

[Unreleased]: https://github.com/evengenius/1c-crserver-manager/compare/v2.3.0...HEAD
[2.3.0]: https://github.com/evengenius/1c-crserver-manager/compare/v2.2.3...v2.3.0
[2.2.3]: https://github.com/evengenius/1c-crserver-manager/compare/v2.2.2...v2.2.3
[2.2.2]: https://github.com/evengenius/1c-crserver-manager/compare/v2.2.1...v2.2.2
[2.2.1]: https://github.com/evengenius/1c-crserver-manager/compare/v2.2.0...v2.2.1
[2.2.0]: https://github.com/evengenius/1c-crserver-manager/compare/v2.1.4...v2.2.0
[2.1.4]: https://github.com/evengenius/1c-crserver-manager/compare/v2.1.3...v2.1.4
[2.1.3]: https://github.com/evengenius/1c-crserver-manager/compare/v2.1.2...v2.1.3
[2.1.2]: https://github.com/evengenius/1c-crserver-manager/compare/v2.1.1...v2.1.2
[2.1.1]: https://github.com/evengenius/1c-crserver-manager/compare/v2.1.0...v2.1.1
[2.1.0]: https://github.com/evengenius/1c-crserver-manager/compare/v2.0.4...v2.1.0
[2.0.4]: https://github.com/evengenius/1c-crserver-manager/compare/v2.0.3...v2.0.4
[2.0.3]: https://github.com/evengenius/1c-crserver-manager/compare/v2.0.2...v2.0.3
[2.0.2]: https://github.com/evengenius/1c-crserver-manager/compare/v2.0.1...v2.0.2
[2.0.1]: https://github.com/evengenius/1c-crserver-manager/compare/v2.0.0...v2.0.1
[2.0.0]: https://github.com/evengenius/1c-crserver-manager/releases/tag/v2.0.0
